...
if(!file.exists(plot_file)) {
	...
	for(i in 1:nrow(GTEx_metadata)) {
		GTEx_tissue = readRDS(GTEx_metadata$accuracyModelEvaluation[i])
		GTEx_tissue$tissue = GTEx_metadata$study[i]
		all_GTEx[[i]] = GTEx_tissue
	}
	all_GTEx = do.call(rbind, all_GTEx)
	all_GTEx$study = "GTEx Test"

	geuvadis = readRDS(Geuvadis_metadata$accuracyModelEvaluation[1])
...
pdf("../figures/Accuracy_model_evaluation_test_set.pdf", width = 8.5, height = 4)
ggplot(GTEx_vs_Geuvadis %>% filter(method == "empirical"), aes(x = coverage, y = prob)) + 
  geom_point(alpha = .85, size = 1, color = "#00BFC4") +
  geom_line(data = GTEx_vs_Geuvadis %>% filter(method == "predicted"), aes(x = coverage, y = prob, group = study), size = 1.1, color = "#F8766D") +
...
pdf("../figures/Accuracy_model_evaluation_test_set_specificTissues.pdf", width = 8.5, height = 7)
ggplot(tissue_comparison %>% filter(method == "empirical"), aes(x = coverage, y = prob)) + 
  geom_point(alpha = .8, size = 1, color = "#00BFC4") +
  geom_line(data = tissue_comparison %>% filter(method == "predicted"), aes(x = coverage, y = prob), size = 1.1, color = "#F8766D") +
  labs(x = "Coverage", y = "P(Correct Genotype)") +
